...
static void fillInBuf(void)
{
	...
	gInBufOfs = 16; // To align with 16 bytes in memcpy()
	gInBufLeft = 0;
...
static void createWinogradQuant(int16 *pQuant)
{
	...
	for (i = 0; i < 64; i++)
	{
		...
		x *= gWinogradQuant[i];
		pQuant[i] = (int16) ((x
			+ (1 << (PJPG_WINOGRAD_QUANT_SCALE_BITS - PJPG_DCT_SCALE_BITS - 1))) >> (PJPG_WINOGRAD_QUANT_SCALE_BITS -	PJPG_DCT_SCALE_BITS));
	}
}
